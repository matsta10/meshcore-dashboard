...
Performs:
1. Adds new columns if missing (fingerprint, parse_status, direction, etc.)
2. Renames 'timestamp' to 'collected_at' if needed
3. Backfills fingerprints and structured fields from raw_line
4. Removes exact duplicate rows (keeping lowest id)
5. Removes junk rows (EOF, empty)
6. Creates unique index on fingerprint
"""
...
def migrate(db_path: str, *, dry_run: bool = False) -> None:
    ...
    if "timestamp" in columns and "collected_at" not in columns:
        cur.execute("ALTER TABLE packet_logs RENAME COLUMN timestamp TO collected_at")
    ...
    junk_deleted = cur.execute(
        "DELETE FROM packet_logs WHERE raw_line LIKE '%EOF%' OR TRIM(raw_line) = ''"
    ).rowcount
    ...
    for row_id, raw_line in rows:
        fp = hashlib.sha256(raw_line.encode()).hexdigest()
